...
export default function HomeScreen() {
  ...
  const chartData = (() => {
    ...
    return months.map((m) => {
      ...
      return {
        month: new Date(y, mo - 1, 1).toLocaleString('en-US', { month: 'short' }),
  ...
  const alertCategories = categories.filter(
    (c) => c.monthlyLimit !== null && c.monthlyLimit > 0 && (c.spent / c.monthlyLimit) >= 0.8,
  );
  ...
  return (
    <ThemedScreen>
      <ScrollView
        ...
        {/* Balance card */}
        <BalanceCard balance={balance} holderName={displayName} accountCount={accounts.length} />

        {/* Month summary */}
        <View style={styles.summaryRow}>
          <View style={[styles.summaryCard, { borderLeftColor: Colors.success, backgroundColor: theme.card }]}>
          ...
          <View style={[styles.summaryCard, { borderLeftColor: Colors.danger, backgroundColor: theme.card }]}>
          ...
          <View style={[styles.summaryCard, { borderLeftColor: Colors.primary, backgroundColor: theme.card }]}>
        ...
        {alertCategories.length > 0 && (
          <TouchableOpacity
            ...
            <Text style={styles.alertText}>
              {alertCategories.length} {alertCategories.length === 1 ? 'category' : 'categories'} near budget limit
            </Text>
          ...
          <View style={[styles.card, { backgroundColor: theme.card }]}>
            {recent.map((tx, i) => (
              <View key={tx.id}>
                ...
                {i < recent.length - 1 && <View style={[styles.divider, { backgroundColor: theme.border }]} />}
              </View>
            ))}
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  summaryRow: { flexDirection: 'row', paddingHorizontal: Spacing.three, gap: Spacing.two },
  summaryCard: { flex: 1, borderRadius: BorderRadius.md, padding: Spacing.two + 2, borderLeftWidth: 3, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 6, shadowOffset: { width: 0, height: 2 }, elevation: 2 },
  summaryLabel: { fontSize: 11, marginBottom: 2 },
  summaryAmt: { fontSize: 14, fontWeight: '700' },
  alertBanner: { flexDirection: 'row', alignItems: 'center', gap: Spacing.two, marginHorizontal: Spacing.three, backgroundColor: Colors.secondary + '15', borderRadius: BorderRadius.md, padding: Spacing.two + 2, borderWidth: 1, borderColor: Colors.secondary + '40' },
  alertText: { flex: 1, fontSize: 13, fontWeight: '500', color: Colors.secondary },
  section: { paddingHorizontal: Spacing.three, gap: Spacing.two },
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  sectionTitle: { fontSize: 16, fontWeight: '700' },
  viewAll: { fontSize: 13, color: Colors.primary, fontWeight: '600' },
  card: { borderRadius: BorderRadius.lg, padding: Spacing.three, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 8, shadowOffset: { width: 0, height: 2 }, elevation: 2 },
  divider: { height: 1 },
});
